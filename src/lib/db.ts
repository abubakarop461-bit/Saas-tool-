// src/lib/db.ts - Cloudflare D1 Query Driver & In-Memory Fallback
...
declare global {
  var __luxeLocalCache: Record<string, any[]> | undefined;
}

function getLocalCache(): Record<string, any[]> {
  if (!globalThis.__luxeLocalCache) {
    globalThis.__luxeLocalCache = {
      leads: [],
      properties: [],
      site_visits: [],
      transactions: [],
  ...
  return globalThis.__luxeLocalCache;
}
...
export async function getD1Record<T = any>(
  ...
  const cache = getLocalCache();
  const cached = cache[table]?.find((r: any) => r.id === id);
...
export async function upsertD1Record(
  ...
  const idx = cache[table].findIndex((r: any) => r.id === record.id);
  if (idx >= 0) {
    cache[table][idx] = { ...cache[table][idx], ...record };
  } else {
