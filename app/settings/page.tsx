...
type SettingSection = 'profile' | 'users' | 'notifications' | 'integrations' | 'security' | 'backups';
...
const sections: { id: SettingSection; label: string; icon: React.ElementType; description: string; superAdminOnly?: boolean }[] = [
  ...
  { id: 'backups', label: 'Database & Backups', icon: Database, description: 'Download complete ERP backups, export table data, and view cloud status.' },
];
...
export default function SettingsPage() {
  ...
  const [backupStats, setBackupStats] = useState<{ leads: number; properties: number; locations: number; images: number; profiles: number } | null>(null);
  const [loadingBackupStats, setLoadingBackupStats] = useState(false);
  const [exportingTable, setExportingTable] = useState<string | null>(null);
  const [exportingFull, setExportingFull] = useState(false);

  // Fetch Backup Stats when Backups tab is active
  useEffect(() => {
    if (activeSection === 'backups') {
      ...
      Promise.all([
        ...
        supabase.from('property_images').select('*', { count: 'exact', head: true }),
        supabase.from('profiles').select('*', { count: 'exact', head: true })
      ]).then(([leadsRes, propsRes, locsRes, imgRes, profRes]) => {
        setBackupStats({
      ...
      }).finally(() => setLoadingBackupStats(false));
    }
  }, [activeSection]);
  ...
  const handleDownloadFullBackup = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `luxe-erp-full-backup-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  const handleDownloadTableCsv = async (tableName: string) => {
    setExportingTable(tableName);
    try {
      const res = await fetch(`/api/backup?table=${tableName}&format=csv`);
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      ...
      console.error('Table CSV export error:', err);
    } finally {
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-[11px] text-zinc-400 mt-0.5">
                    Download full ERP snapshots, export tables as CSV/JSON, and review cloud disaster recovery safeguards.
                  </p>
                </div>
                ...
                {/* Live Database Record Overview */}
                <div className="border border-[#e8e7e4] rounded-2xl p-5 bg-[#fafaf8]">
                  <div className="flex items-center justify-between mb-3">
                    <div className="text-[10px] font-extrabold text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
                      <Server className="h-3.5 w-3.5 text-zinc-500" />
                      Live Database Inventory
                    </div>
                    {loadingBackupStats && (
                      <span className="text-[10px] text-zinc-400 font-bold flex items-center gap-1">
                        <RefreshCw className="h-3 w-3 animate-spin" /> Counting records...
                      </span>
                    )}
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                    <div className="bg-white border border-[#e8e7e4] rounded-xl p-3 text-center shadow-2xs">
                      <div className="text-[10px] font-bold text-zinc-400">Leads</div>
                      <div className="text-lg font-black text-zinc-900 mt-0.5">{backupStats ? backupStats.leads : '—'}</div>
                    </div>
                    <div className="bg-white border border-[#e8e7e4] rounded-xl p-3 text-center shadow-2xs">
                      <div className="text-[10px] font-bold text-zinc-400">Properties</div>
                      <div className="text-lg font-black text-zinc-900 mt-0.5">{backupStats ? backupStats.properties : '—'}</div>
                    </div>
                    <div className="bg-white border border-[#e8e7e4] rounded-xl p-3 text-center shadow-2xs">
                      <div className="text-[10px] font-bold text-zinc-400">Locations</div>
                      <div className="text-lg font-black text-zinc-900 mt-0.5">{backupStats ? backupStats.locations : '—'}</div>
                    </div>
                    <div className="bg-white border border-[#e8e7e4] rounded-xl p-3 text-center shadow-2xs">
                      <div className="text-[10px] font-bold text-zinc-400">Images</div>
                      <div className="text-lg font-black text-zinc-900 mt-0.5">{backupStats ? backupStats.images : '—'}</div>
                    </div>
                    <div className="bg-white border border-[#e8e7e4] rounded-xl p-3 text-center shadow-2xs">
                      <div className="text-[10px] font-bold text-zinc-400">Profiles</div>
                      <div className="text-lg font-black text-zinc-900 mt-0.5">{backupStats ? backupStats.profiles : '—'}</div>
                    </div>
                  </div>
                </div>

                {/* Primary Action: Full Database Dump */}
                <div className="border border-[#e8e7e4] rounded-2xl p-5 bg-white space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <h3 className="text-xs font-extrabold text-zinc-900 flex items-center gap-1.5">
                        <HardDrive className="h-4 w-4 text-[#d4ad4d]" />
                        Download Complete System Snapshot (JSON)
                      </h3>
                      <p className="text-[10.5px] text-zinc-400 mt-0.5">
                        Includes all leads, properties, property images, locations, settings, team profiles, site visits, and audit logs.
                      </p>
                    </div>
                    <button
                      type="button"
                      ...
                      className="dc-btn gold font-bold text-xs px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 cursor-pointer shrink-0 shadow-xs disabled:opacity-50"
                    >
                      <Download className="h-3.5 w-3.5" />
                      {exportingFull ? 'Generating Backup...' : 'Download Full Backup'}
                    </button>
                  </div>
                </div>

                {/* Table-by-Table CSV Exports */}
                <div className="border border-[#e8e7e4] rounded-2xl p-5 bg-white space-y-4">
                  <div>
                    <h3 className="text-xs font-extrabold text-zinc-900 flex items-center gap-1.5">
                      <FileSpreadsheet className="h-4 w-4 text-emerald-600" />
                      Export Individual Tables (Excel / CSV)
                    </h3>
                    <p className="text-[10.5px] text-zinc-400 mt-0.5">
                      Download individual clean CSV spreadsheets for reporting or offline analysis.
                    </p>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {[
                      { key: 'leads', label: 'Leads Table', desc: 'All client contacts & pipeline status' },
                      { key: 'properties', label: 'Properties Inventory', desc: 'All residential & commercial listings' },
                      { key: 'locations', label: 'Locations Directory', desc: 'All 27 master Pune localities' },
                      { key: 'profiles', label: 'Team Members', desc: 'Agents, admins, and roles' },
                      { key: 'site_visits', label: 'Site Visits', desc: 'All past and scheduled visits' },
                      { key: 'audit_logs', label: 'Audit Activity Logs', desc: 'System events & change logs' },
                    ].map(t => (
                      <div key={t.key} className="border border-[#e8e7e4] rounded-xl p-3.5 flex flex-col justify-between bg-[#fafaf8] hover:bg-zinc-50 transition-colors">
                        <div>
                        ...
                        <button
                          type="button"
                          onClick={() => handleDownloadTableCsv(t.key)}
                          disabled={exportingTable === t.key}
                          className="mt-3 w-full py-1.5 px-3 bg-white hover:bg-zinc-100 border border-[#e8e7e4] rounded-lg text-[11px] font-bold text-zinc-700 flex items-center justify-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
                        >
                          <Download className="h-3 w-3 text-zinc-500" />
                          {exportingTable === t.key ? 'Exporting...' : 'Export CSV'}
                        </button>
                      </div>
                    ))}
                  </div>
                ...
                {/* Automated API / Cron Integration Info */}
                <div className="p-4 bg-zinc-50 border border-zinc-200/80 rounded-2xl text-xs space-y-1.5">
                  <div className="font-extrabold text-zinc-900 flex items-center gap-1.5">
                    <Globe className="h-3.5 w-3.5 text-zinc-500" />
                    Automated Webhook / Off-Site Cron URL
                  </div>
                  <p className="text-[11px] text-zinc-500">
                    You can schedule automated off-site backups directly via HTTP request or Google Apps Script. Calling <code className="bg-zinc-200/70 px-1.5 py-0.5 rounded text-[10px] font-mono text-zinc-800">/api/backup</code> downloads the live JSON payload.
                  </p>
                </div>
